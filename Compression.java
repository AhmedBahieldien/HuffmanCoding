...
class FrequencyComparator implements Comparator<TreeNode> {
...
public class Compression {
    ...
    void writeInFolder(String inputString, int n)
    {
        try {
            ...
            for(int i = 0; i<n; i++)
            {
                outputStringBuilder.append(codeMap.get(inputString.charAt(i)));
            }
            writeOutput.write(decodingStringBuilder.toString() + outputStringBuilder.toString());
            writeOutput.close();
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    {
        float i = inputStringBuilder.toString().length()*8;
